...
const SocialLogin = ({ navigation, setSuccess, setError, setLoading, deviceToken }: SocialLoginProps, ref) => {
  const { setUserInfo, setIsAuthenticated, updateUserInfoAndAuthenticate } = useContext(UserContext);
  // const [deviceToken, setDeviceToken] = useState('');

  console.log("insocial",deviceToken)
  ...
  const processLogin = async (provider: 'google' | 'facebook' | 'apple', data: any) => {
    try {
      ...
      if (provider === 'google') {
        ...
        if (Platform.OS === 'ios' && data.tokenType === 'authentication') {
          // iOS with Authentication Token
          requestData = {
            provider: 'facebook',
  ...
  //   //   console.log("Access token obtained:", data.accessToken);
  ...
  //   //   setError(`Error while logging in with Facebook: ${error.message || 'Unknown error'}`);
  //   //   setLoading(false);
  //   // }
  // };
  ...
  const onFacebookButtonPress = async () => {
    try {
      ...
      // Handle different token types for iOS vs Android
      if (Platform.OS === 'ios') {
        // iOS uses Authentication Token
        const authResult = await AuthenticationToken.getAuthenticationTokenIOS();
        if (!authResult) {
          throw new Error('Failed to obtain authentication token');
        }
        
        console.log("iOS Authentication token obtained:", authResult.authenticationToken);
        
        // Create a data object compatible with processLogin for iOS
        tokenData = {
        ...
        // Android uses Access Token
        const accessResult = await AccessToken.getCurrentAccessToken();
        if (!accessResult) {
          throw new Error('Failed to obtain access token');
        }
        
        console.log("Android Access token obtained:", accessResult.accessToken);
        
        // Use the existing access token format for Android
      ...
      // Process the login with the server
      await processLogin('facebook', tokenData);
      
    } catch (error: any) {
      console.error('Facebook Login Error:', error);
      //${error.message || 'Unknown error'}
      setError(`Error while logging in with Facebook`);
      setLoading(false);
    }
  };
  ...
  const onGoogleButtonPress = async () => {
    try {
      ...
      if (error.code === statusCodes.SIGN_IN_CANCELLED) {
        ...
        setError(`Error while logging in with Google`);
      }
    }
  };
  ...
  const onAppleButtonPress = async () => {
    ...
    try {
      ...
      if (error.code === appleAuth.Error.CANCELED) {
        ...
        setError(`Error while logging in with Apple`);
      }
...
export default forwardRef(SocialLogin);