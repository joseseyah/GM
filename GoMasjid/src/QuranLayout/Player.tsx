...
// import {loadAudioJsonFile, loadAudioJuzMapJsonFile} from '../services/fileCache';
...
const Player = ({ metaData, type, trackChange }: any) => {
  ...
  const {isPlaying, playFromItem, pausePlaying, trackIndex} = playerStore();
  const [fromBotttom, setFromBottom] = useState<boolean>(false);
  const [downloadIndex, setDownloadIndex] = useState<any>(0);
  ...
  const togglePlay = async () => {
    ...
    try {
      ...
      if (state === State.Playing) {
        ...
        setIsLoading(true);
        // console.log("playFromItem else", playFromItem);
        if (state === State.Ready || state === State.Paused) {
          ...
          if (playFromItem != null) {
            // console.log("playFromItem next inside if", playFromItem);
            if(fromBotttom && state != State.Paused) {
              // console.log("from bottom", fromBotttom);
              setFromBottom(false);
              await TrackPlayer.play();
            } else {
              // console.log("from bottom else", fromBotttom);
            ...
            // console.log("else lastss");
            if (currentTrack === 0 && queue.length > 0) {
              // console.log("else lastss 1");
  ...
  const waitForDownload = async () => {
    ...
    while (playerStore.getState().trackIndex <= playFromItem) {
  ...
  const handleDownload = useCallback(async () => {
    ...
    try {
      ...
      // const audioFiles = await loadAudioJsonFile(reciter_slug, type, metaData.number);

      if (audioFiles?.verses) {
        setAudioData(audioFiles?.verses);
        if (type === 'juz') {
          const juzMapData = typedJuzMapping[metaData.number];
          // const juzMapData = await loadAudioJuzMapJsonFile(metaData.number);
      ...
      for (let i=0; i < audioData.length; i++) {
        if (settings.player?.currentSurahID === metaData?.number && settings.player?.currentPlayType === type) {
          ...
          if (settings.player?.currentSurahID === metaData?.number && settings.player?.currentPlayType === type) {
            ...
            playerStore.setState({trackIndex: i})
            setDownloadIndex(i);
            // console.log("added", i);
